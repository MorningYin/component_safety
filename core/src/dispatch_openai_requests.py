...
# 支持通义千问 API (DashScope)
api_key = os.environ.get("OPENAI_API_KEY", "sk-3cab163569024c3781464a6c549d0c98")
base_url = os.environ.get("OPENAI_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")

# 创建不使用代理的 httpx 客户端（用于 DashScope API，国内访问不需要代理）
# HuggingFace 仍然使用系统代理
no_proxy_http_client = httpx.AsyncClient(proxy=None, timeout=60.0)
no_proxy_sync_http_client = httpx.Client(proxy=None, timeout=60.0)

aclient = AsyncOpenAI(api_key=api_key, base_url=base_url, http_client=no_proxy_http_client)
client = OpenAI(api_key=api_key, base_url=base_url, http_client=no_proxy_sync_http_client)
import asyncio
