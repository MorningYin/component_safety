...
from evaluation.tasks.metrics import get_average, get_f1, get_all_metrics, get_confusion_matrix
...
class ToxicChatPromptSafetyEvalBase(ClassificationTaskBase, abc.ABC):
    ...
    def _evaluate(self, model: SafetyClassifierBase) -> tuple[dict, list[dict]]:
        ...
        # 计算基础指标
        base_metrics = {k: get_average(v) if "f1" not in k else get_f1(v) for k, v in
                        pred_is_correct.items()}
        
        # 计算所有详细指标（基于 f1 数据）
        if "f1" in pred_is_correct and len(pred_is_correct["f1"]) > 0:
            all_metrics = get_all_metrics(pred_is_correct["f1"])
            # 合并到基础指标中
            base_metrics.update(all_metrics)
        
        return base_metrics, individual_results
