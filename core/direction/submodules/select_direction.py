...
from components_safety.core.pipeline_utils.hook_utils import add_hooks, get_activation_addition_input_pre_hook, get_direction_ablation_input_pre_hook, get_direction_ablation_output_hook
...
def get_refusal_scores(model, instructions, tokenize_instructions_fn, refusal_toks, fwd_pre_hooks=[], fwd_hooks=[], batch_size=32, desc=None):
    ...
    if desc is not None:
        it = tqdm(range(0, len(instructions), batch_size), desc=desc)
    else:
...
def get_last_position_logits(model, tokenizer, instructions, tokenize_instructions_fn, fwd_pre_hooks=[], fwd_hooks=[], batch_size=32) -> Tensor:
...
def filter_fn(refusal_score, steering_score, kl_div_score, layer, n_layer, kl_threshold=None, induce_refusal_threshold=None, prune_layer_percentage=0.20) -> bool:
    if math.isnan(refusal_score) or math.isnan(steering_score) or math.isnan(kl_div_score):
        return True
    if prune_layer_percentage is not None and n_layer is not None and layer >= int(n_layer * (1.0 - prune_layer_percentage)):
        return True
...
def select_direction(
    ...
    for source_pos in range(-n_pos, 0):
        for source_layer in tqdm(range(n_layer), desc=f"Computing refusal addition for source position {source_pos}"):
            ...
            # Use a slightly higher coeff to ensure refusal signal is visible in logit space
...
def select_cone_basis(
    ...
    # Simplified sampling logic for local execution without wandb config
    ...
    for candidate_idx in range(n_candidates):
        ...
        # alpha default to 1.0 if not specified
        alpha = 1.0
    ...
    print(f"Mean Refusal score: {sample_ablation_refusal_scores[candidate_idx].mean().item():.4f} ± {sample_ablation_refusal_scores[candidate_idx].std().item():.4f}")
    print(f"Mean Steering score: {sample_steering_refusal_scores[candidate_idx].mean().item():.4f} ± {sample_steering_refusal_scores[candidate_idx].std().item():.4f}")
    print(f"Mean KL Divergence score: {sample_ablation_kl_div_scores[candidate_idx].mean().item():.4f} ± {sample_ablation_kl_div_scores[candidate_idx].std().item():.4f}")
...
def kl_div_fn(
    ...
    Args:
        logits_p: Baseline logits [batch, d_vocab]
        logits_q: Intervention logits [batch, d_vocab]
        mask: Optional mask for tokens to consider
    """
    p = torch.nn.functional.softmax(logits_p, dim=-1)
    q = torch.nn.functional.softmax(logits_q, dim=-1)

    kl = p * (torch.log(p + epsilon) - torch.log(q + epsilon))
    kl = kl.sum(dim=-1)
