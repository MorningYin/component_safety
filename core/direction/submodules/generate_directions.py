...
from components_safety.core.pipeline_utils.hook_utils import add_hooks
from components_safety.core.models.base import ModelBase

def get_mean_activations_pre_hook(layer, cache: Tensor, n_samples: int, positions: List[int]):
    def hook_fn(module, input):
        activation = input[0].clone().to(cache)
        cache[:, layer] += (1.0 / n_samples) * activation[:, positions, :].sum(dim=0)
    return hook_fn

def get_mean_activations(model, tokenizer, instructions, tokenize_instructions_fn, block_modules, batch_size=32, positions=[-1]):
...
def get_mean_diff(model, tokenizer, harmful_instructions, harmless_instructions, tokenize_instructions_fn, block_modules, batch_size=32, positions=[-1]):
